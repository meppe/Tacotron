...
from datasets import blizzard, ljspeech, wav2wav
from hparams import hparams


def preprocess_blizzard(args):
  in_dir = os.path.join(args.base_dir, 'Blizzard2012')
  out_dir = os.path.join(args.base_dir, args.output + "/" + args.dataset)
...
def preprocess_wav2wav(args):
  in_dir = os.path.join(args.base_dir, 'wav2wav')
  out_dir = os.path.join(args.base_dir, args.output + "/" + args.dataset)
  os.makedirs(out_dir, exist_ok=True)
  metadata = wav2wav.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)
...
def write_metadata(metadata, out_dir):
  ...
  print('Wrote %d utterances, %d target frames (%.2f hours)' % (len(metadata), frames, hours))
...
def main():
  ...
  elif args.dataset == 'wav2wav':
    preprocess_wav2wav(args)
