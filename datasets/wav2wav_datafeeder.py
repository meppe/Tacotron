...
class Wav2WavDataFeeder(threading.Thread):
  '''Feeds batches of data into a queue on a background thread.'''

  def __init__(self, coordinator, metadata_filename, hparams):
    super(Wav2WavDataFeeder, self).__init__()
    ...
    self._placeholders = [
      tf.placeholder(tf.float32, [None, None, hparams.num_src_freq], 'inputs'),
...
def _prepare_inputs(inputs):
  ...
  padded_input_list = [_pad_input(x, max_len) for x in inputs]
  return np.stack(padded_input_list)
